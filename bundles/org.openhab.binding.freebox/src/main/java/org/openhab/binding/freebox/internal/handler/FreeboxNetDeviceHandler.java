...
public class FreeboxNetDeviceHandler extends BaseThingHandler {

    private final Logger logger = LoggerFactory.getLogger(FreeboxNetDeviceHandler.class);

    private FreeboxHandler freeboxHandler;

    private ScheduledFuture<?> refreshTask;

    private FreeboxNetDeviceConfiguration config;

    public FreeboxNetDeviceHandler(Thing thing) {
        super(thing);
    ...
    public void initialize() {
        ...
        config = getConfigAs(FreeboxNetDeviceConfiguration.class);
    ...
    private void startAutomaticRefresh() {
        if (refreshTask == null || refreshTask.isCancelled()) {
            refreshTask = scheduler.scheduleWithFixedDelay(this::updateNetDevice, 1, getConfigAs(FreeboxNetDeviceConfiguration.class).refreshInterval, TimeUnit.SECONDS);
        }
    }
